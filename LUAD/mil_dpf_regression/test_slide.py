...
parser.add_argument('--num_bags_per_slide', default='4', type=int, help='Number of bags to be inferred to obtain slide-level tumor purity', dest='num_bags_per_slide')
parser.add_argument('--test_metrics_dir', default='test_metrics_slide', help='Text file to write test metrics', dest='test_metrics_dir')
...
dataset = Dataset(image_dir=FLAGS.image_dir, normal_image_dir=FLAGS.normal_image_dir, dataset_dir=FLAGS.dataset_dir, dataset_type=FLAGS.dataset_type, patch_size=FLAGS.patch_size, fold_list=fold_list, num_instances=FLAGS.num_instances, num_bags_per_slide= FLAGS.num_bags_per_slide)
